...
\begin{proof}[Dowsód (A)]
  Chcemy zdefiniować liniową izometrię $S\colon (c_o)^* \overset{\textrm{na}}{\longrightarrow} l_1$.
	...
	Chcemy pokazać, że $S\colon (c_0)^* \to l_1$ jest liniową suriekcją oraz jeśli $\varphi(x) = \sum_i x_i c_i$  gdzie $x=(x_1, x_2, \ldots)$ to $S(\varphi) = (c_1,c_2,\ldots)$.\par
	Niech $\varphi \in (c_0)^*$ i~$\varphi(e_i) = c_i$. 
	Wtedy $v_n = \frac{\overline{c_1}}{|c_1|}e_1 + \ldots+ \frac{\overline{c_n}}{|c_n|}e_n \in c_0.$ i $\|v_n\|_\infty =1$.
