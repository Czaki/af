...
\geometry{verbose,a4paper,tmargin=2.5cm,bmargin=2.5cm,lmargin=3.0cm,rmargin=3.0cm}
...
\author{Roman Pol}
\newtheorem{twr}[subsubsection]{Twierdzenie}%[section]
...
Niech $X$ będzie  przestrzenią liniowa nad $\R$ lub $\C$. Niech $A,B \subset  X$, $s,t \in K$ wtedy:
$$sA+tB = \{su + tv: u \in A, v\in B\}$$
$u_1,\ldots ,u_n \in X $. $\lin\{u_1,\ldots, u_n\} = \K u_1 + \ldots + \K u_n $ gdzie $\K u = \{ t u: t \in \K\} $ 

Niech $g: X_\R \to \R $ funkcjonał liniowy. Wówczas $f: X \to \C$, $f(x) =g(x) - ig(ix)$, $f$ jest liniowe ($f(\alpha x) = \alpha f(x)$). 
...
$X$ -- przestrzeń liniowa nad $\K$. Normą nazywamy funkcję  $\|\cdot \| : X \to [0, +\infty )$ taką, że:
	...
	\item $\|x+y\| \le \|x\| + \|y\|$  
\end{enumerate}
$(X,\|\cdot\|)$ -- przestrzeń unormowana jest przestrzenią liniową nad $X$ z~normą $\|\cdot\|$\\
...
\begin{description}
 	 \item[$( l_\infty(S,K), \|\cdot \|_\infty)$:] Niech $S$ będzie zbiorem. Wtedy $l_\infty (S,K)$ jest zbiorem funkcji ograniczonych $X:S\to K$ 
		...
		Niech $\|X\|_\infty = \sup \{|x(s): s\in S\} < \infty$.\\
		Przyjmujemy $(l_\infty, \|\cdot\|_\infty) = ( l_\infty(\N,\C), \|\cdot\|_\infty )$ 
		-- przestrzeń ciągów zespolonych z~normą ,,supremum''.
	  \item[$(c_0, \|\cdot\|)$:] podprzestrzeń $(l_\infty, \|\cdot \| )$ złożona z~ciągów zespolonych zbieżnych do zera.
...
Mówiąc o~tolopogii w~$(X,\|\cdot\|)$ mamy na myśli topologię wyznaczoną przez metrykę.\\
...
\begin{lem}
  \label{lem1}
	$(X,\|\cdot\|)$ -- unormowana nad ciałem $\K$. $Y\subset X$ podprzestrzeń liniowa domknięta. $u\in X\setminus Y$ 
	\begin{enumerate}[(i)]
...
\begin{proof}
	\begin{enumerate}[(i)]
	  \item $y\in Y$ i~$\lambda \in \K\setminus \{0\}$ mamy $\|y+\lambda u\|= |\lambda|\cdot \left\| u - \frac{-y}{\lambda}\right\|\ge |\lambda| \cdot \dist(u,Y)$.
		\item $\Rightarrow$. Niech $y_n + \lambda_n u \to z$ wtedy  $(y_n + \lambda_n u )_n $ jest ciągiem Cauchego. 
			Z~\textit{(i)} wynika, że $(\lambda_n)_n$ też jest ciągiem Cauchego.
...
\begin{twr}
	...
	\begin{enumerate}[(i)]
		\item Jeśli $Y\subset X$ - domknięta, linowa, $Z\subset X$ skończenie wymiarowa to $Y+ Z$ jest domknieta.
		\item Jeśli $X$ jest $n$-wymiarowa, to $(X,\|\cdot\|)$ jest linowo\dywiz homeomorficzna z~$(\K^n, \|\cdot\|_{max})$.
...
\end{proof}
\textit{Przypominienie definicji:}
...
\begin{lem} \label{lem:Riesz1}
  Niech $F\colon[0,1]\to [0,+\infty)$ będzie funkcją niemalejącą. Niech $F_l(t) = \lim_{s\to t^-} F(s)$ dla $t\in[0,1]$. Istnieje wtedy miara Borelowska $\mu$ na $[0,1]i$
...
\begin{proof}
	%	\texttt{rysunek} \\ tu miał być rysunek  f jako funkcji schodkowej.
			...
			Zdefiniujmy $\mu$ na $[0,1]$ przez $\mu(A) = \lambda(G^{-1}(A))$ gdzie $\lambda$ jest miarą Lebesgua.
...
\begin{proof}
	...
	Jeśli $s<t$ to $\chi_{[0,s)} \le \chi_{[0,t]}$ więc $\phi\left( \chi_{0,s)} \right) \le \phi \left( \chi_{[0,t)} \right)$
	  Ponieważ $\psi$ jest nieujemne to $F$ jest niemalejące i~mamy $\mu$ jak w~poprzednim lemacie (\ref{lem:Riesz1}).\\
...
\begin{proof}
	...
	Dlatego, jeśli $\varphi\not = 0$ istnieje $v\ge 0$ dla którego $\varphi(v)>0$.
...
\begin{proof}[Dowód twierdzenia Riesza]
	$\varphi: C[0,1] \to \R$, nieujemny, linowy funkcjonał.
	$\left( C[0,1],\norm_\infty \right) \subset\left( l_\infty (I,\R),\norm_\infty \right)$
	$C=\{ u\in l_\infty (I,\R): \inf_{t \in I} u(t) >0$ -- zbiór wypukły i~otwarty. 
	$C\cap C[0,1] = \{ f \in C[0,1]: f>0\}$.\par
	$\varphi:C[0,1] \to \R$ liniowy nieujemny wtedy $\varphi$ jest dodatnie na $C\cap C[0,1]$. Z~Uwagi 1.3.6.
	istnieje ciągłe liniowe rozszerzenie $\psi:\l_\infty(I,\R) \to R$ dodatni na C. \par
	\emph{Zauważmy: jeśli $v\ge 0$ $(v \in l_\infty (I,\R)$  wtedy $\psi(v)>0$. Niech $\mu$ będzie miarą jak w~Lemacie 1.4.3.}\par
	Musimy sprawdzić dla $f\in C[0,1]$, ze $\varphi(f) = \int_{[0,1]}f \dd \mu$.
	Ustalmy $\varepsilon>0$. Dopóki $f$ jest jednostajnie ciągła możemy zastąpić $[0,1]$ przez punkty $0<t_1<\ldots<t_n<1$\texttt{[\dots]}\par
	$g$  jest funkcją schodkową 
	$$g= f(0)\cdot \chi_{[0,t_1)} + \sum_{i=1}^{n-1}f(t_i) \chi_{[t_i,t_{i+1}} + f(t_n) \chi_{[t_n,1]}$$
	Wtedy $\left|\varphi(f) - \int_{[0,1]} f \dd \mu\right| = \left| \psi(f) - \psi(g) +\psi(g) - \int_{[0,1]} \right|
	\le \left| \psi(f-g)\right| + \left| \int_{[0,1]} g \dd \mu - \int_{[0,1]}\right|$ \texttt{\dots}
\end{proof}
